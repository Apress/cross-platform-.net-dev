...
namespace Crossplatform.NET.Chapter09
{
    public class GtkView : View
    {
        ...
        public GtkView(Person person, Controller controller) : base(person, controller)
        {
            Application.Init();
        ...
        private class GtkViewWindow : Dialog
        {
            ...
            public GtkViewWindow(string title, Window parentWindow, DialogFlags flags, Person person):base(title, parentWindow, flags)
            {
                this.person = person;
                
                Label nameLabel = new Label(this.person.Firstname + " " + this.person.Surname);
